...
class Task008SafeBuyerIntegrationTest {
    ...
    @Test void canonicalTypedJourneyUsesReadyManifestGroundedProductsAuthoritativeQuoteAndCertificate(){Fixture f=fixture("canonical");CommerceThread thread=threads.create(f.buyer().id(),canonicalText());
        ...
        assertThat(jdbc.sql("SELECT count(*)::int FROM buyer_agent_action WHERE thread_id=:thread AND provider_name='DETERMINISTIC_STATE_MACHINE'")
                .param("thread",thread.threadId()).query(Integer.class).single()).isEqualTo(6);
    ...
    @Test void merchantWithoutReadyAdvertisedSearchCapabilityIsExcluded(){Fixture ready=fixture("search-ready");Fixture unready=fixture("search-unready");
        jdbc.sql("UPDATE agent_commerce_manifest_capability SET readiness='UNTESTED',advertised=false WHERE merchant_id=:m AND capability='SEARCH_PRODUCTS'")
                .param("m",unready.merchant().id()).update();CommerceThread thread=threads.create(ready.buyer().id(),canonicalText());
        orchestration.advance(ready.buyer().id(),thread.threadId());orchestration.advance(ready.buyer().id(),thread.threadId());
        assertThat(threads.discovery(ready.buyer().id(),thread.threadId()).eligibleMerchants())
                .extracting(MerchantCandidate::merchantId).contains(ready.merchant().id()).doesNotContain(unready.merchant().id());}
    ...
    @TestConfiguration static class Fakes {
        @Bean @Primary BuyerIntentCompiler buyerIntentCompiler(){return (message,feedback)->{String text=message.normalizedText();int end=text.length();EvidenceSpan span=new EvidenceSpan(message.messageId(),0,end);
            ...
            return new CompiledIntent(IntentGoal.PURCHASE_FOOD,"Snacks",50_000L,"INR",null,null,null,true,"PEANUT",null,2,SubstitutionPolicy.UNKNOWN,null,List.of("HIGH_PROTEIN"),
                    List.of(new MaterialField("BUDGET",ConstraintClassification.HARD,span,new BigDecimal("0.99"),AmbiguityState.CLEAR),new MaterialField("VEGETARIAN",ConstraintClassification.HARD,span,new BigDecimal("0.99"),AmbiguityState.CLEAR),new MaterialField("ALLERGEN",ConstraintClassification.HARD_SAFETY,span,new BigDecimal("0.99"),AmbiguityState.CLEAR),new MaterialField("PREFERENCES",ConstraintClassification.SOFT,span,new BigDecimal("0.9"),AmbiguityState.CLEAR)),AmbiguityState.CLEAR,null,"FAKE","intent-v1");};}
