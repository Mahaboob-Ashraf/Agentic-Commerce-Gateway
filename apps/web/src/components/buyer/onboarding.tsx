...
export function Onboarding() {
  ...
  function enterBuyer() {
    if (!actor || !status?.profileComplete || !status.addressSelected) return;
    localStorage.setItem(`amana:onboarding-complete:${actor.actorId}`, "true");
    localStorage.setItem(`amana:tour-pending:${actor.actorId}`, "true");
    router.push("/buyer/chat?tour=1");
  }

  return <main className={styles.page}>
    <header className={styles.topbar}><button className={styles.wordmark} onClick={() => router.push("/")} type="button"><span>A</span>Amana</button><button className={styles.exit} onClick={() => router.push("/")} type="button">Exit setup</button></header>
