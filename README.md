...
| **Hybrid-v3 discovery Recall@5** | **83.58%** | Candidate discovery on the fixed 80-case labelled Amazing catalogue evaluation |
| **Hybrid-v3 valid-product Recall@5** | **76.12%** | Products that actually qualify for Buyer cart construction, not merely related alternatives |
| **Hybrid-v2 valid-product Recall@5** | **55.22%** | Pre-fix valid-match baseline on the same 80 labelled cases |
| **Valid-product recall improvement** | **+20.90 percentage points** | The bounded semantic qualification lane recovered legitimate semantic matches without lowering the global hybrid threshold |
| **Lexical-only Recall@5** | **53.73%** | Retrieval without vector assistance |
| **Semantic valid retrieval** | **16.67% → 58.33%** | Semantic requests became materially more usable after hybrid-v3 |
| **Generic/category valid retrieval** | **42.86% → 71.43%** | More category-level requests reached executable grounded matches |
| **Typo / ASR-style valid retrieval** | **83.33% → 100%** | Labelled spelling and speech-recognition-style variations reached valid matches |
| **Exact-identity precision** | **100%** | Explicit identity constraints remained exact in the measured retrieval set |
...
Healthy embeddings and an improved retrieval benchmark still left a real Buyer request broken:

`wireless earphones` failed to retrieve `Auralink Buds Bluetooth Earphones`, while the more lexical

`bluetooth earphones` worked. The ranker assigned vectors 30% and completeness 5%, but required a

`0.42` valid-match score. A candidate supported entirely by semantic evidence could score at most

`0.35`. The vector system could work correctly while the scoring architecture prevented qualification.

Hybrid-v3 keeps the weights and `0.42` threshold. When **no candidate in a merchant search qualifies

through the normal hybrid score**, a separate semantic lane can nominate an active product from the authoritative current

catalogue. It requires an active vector provider, READY embeddings, successful vector lookup, cosine

similarity **≥ 0.60**, a `PASS` identity gate, compatible hard category, and passing deterministic price

and food-safety filters. Explicit SKU, GTIN, brand, variant, size/storage, or colour requirements make

the request ineligible for this lane. Cart construction still re-runs retrieval and reloads the exact

authoritative product; inventory, serviceability, authorization and payment gates are unchanged.
...
and live Gemini embeddings, with query vectors shared across paired runs. An initial 0.65–0.95 sweep

missed useful matches; observed scores prompted refinement down to 0.50. Cutoffs 0.50 and 0.55 reduced

valid no-match accuracy to 20% and 60% and were rejected. **0.60 gave the best valid recall among safe

grid rows**; 0.625 and 0.65 were safer than those low cutoffs but recovered fewer legitimate requests.

An earlier candidate-wise lane also admitted headphones alongside an already-correct earbuds result.

That [rejected experiment](proof/results/history/semantic-candidate-wise-experiment.json) motivated the

no-existing-hybrid-match condition; the final lane introduces **zero unexpected valid products**.

The evaluator exposed its own blind spot: historical discovery metrics included related alternatives,
...
**53.73%**. [Historical hybrid-v2 evidence](proof/results/history/retrieval-hybrid-v2.json) is preserved;

the [current generated report](proof/results/RETRIEVAL_EVAL.md) includes the paired comparison and all

12 grid thresholds. These are fixture measurements and calibration, not held-out validation or

production-scale benchmarking.

The focused regression first failed against hybrid-v2 with a mocked 0.95 cosine score, then passed

with hybrid-v3: **`wireless earphones under 3500` → `AMZ-AUDIO-032`**. The separate live evaluation also
