...
function buildIndex(requireOffline) {
  ...
  const rows = [
    ["Backend tests", backendTests ? `${backendTests.tests} tests; ${backendTests.failures} failures; ${backendTests.errors} errors; ${backendTests.skipped} skipped` : "NOT_AVAILABLE", "Canonical Maven test result, kept separate.", "It is not a frontend or proof count.", ".\\apps\\backend\\mvnw.cmd test"],
    ...
    ["Retrieval", `${evidence.retrieval.status}; baseline R@5=${retrievalSummary?.baseline?.recallAt5 ?? "?"}; lexical post-fix R@5=${retrievalSummary?.postFixLexicalOnly?.recallAt5 ?? "?"}; hybrid=${retrievalSummary?.hybridStatus ?? "?"}`, "Labelled Amazing retrieval quality when executed.", "It does not establish unlabelled or production-traffic quality.", evidence.retrieval.reproductionCommand],
